                ...
                if can_service_accept_job():
                    ...
                    if dispatch_result is True:
                        ...
                        if should_retry and retry_count < MAX_RETRIES:
                            ...
                            logging.warning(f"⚠️ Job {job_id} failed ({failure_type}), retrying in {RETRY_DELAY_SECONDS}s (attempt {retry_count + 1}/{MAX_RETRIES})")
                    ...
                    logging.info(f"⏸️ Service at capacity, leaving job {job_id} in queue for KEDA scaling")
        ...
        if job_id:
            with job_lock:
                if job_id in current_jobs:
                    ...
                    logging.warning(f"⚠️ Received callback for unknown job {job_id}")
...
def main():
    logging.info("🎛️ Starting GenoTools Controller (Queue Consumer + Job Dispatcher)...")
